...
void ALadder::OnConstruction(const FTransform& Transform)
{
	...
	{
		float StepsLength = StepsMesh->GetBoundingBox().GetSize().Y;
		if (!FMath::IsNearlyZero(StepsLength))
		{
			StepsMeshComponent->SetRelativeScale3D(FVector(1.f, LadderWidth / StepsLength, 1.f));
		}
	}
...
FVector ALadder::GetAttachFromTopAnimMontageStartingLocation() const
{
	...
	FVector Offset = OrientationRotation.RotateVector(AttachFromTopAnimMontageInitOffset);
